...
public class ComplaintService {
  ...
  @Transactional
  public ComplaintDto create(UUID submittedByUserId, CreateComplaintRequest request) {
  ...
  public List<ComplaintDto> queue(IdentityMeResponse identity) {
    ...
    if (!"authority".equalsIgnoreCase(identity.userType()) && !"developer_admin".equalsIgnoreCase(identity.userType())) {
    ...
    switch (level) {
      ...
      case "PROVINCIAL" -> {
        if (identity.provinceId() == null) throw new BadRequestException("province scope missing");
        complaints = complaintRepository.findByIncidentProvinceIdOrderByCreatedAtDesc(identity.provinceId());
      }
      case "DSD" -> {
        if (identity.districtId() == null) throw new BadRequestException("district scope missing");
        complaints = complaintRepository.findByIncidentDistrictIdOrderByCreatedAtDesc(identity.districtId());
      }
      case "DS" -> {
        if (identity.dsDivisionId() == null) throw new BadRequestException("ds scope missing");
        complaints = complaintRepository.findByIncidentDsDivisionIdOrderByCreatedAtDesc(identity.dsDivisionId());
      }
      case "GS" -> {
        if (identity.gnDivisionId() == null) throw new BadRequestException("gn scope missing");
  ...
  @Transactional
  public ComplaintDto updateStatus(IdentityMeResponse actorIdentity, UUID complaintId, ComplaintStatus toStatus, String note) {
    UUID actorUserId = requireAuthority(actorIdentity);
  ...
  private void ensureCanViewInQueue(IdentityMeResponse identity, Complaint complaint) {
    ...
    switch (level) {
      ...
      case "PROVINCIAL" -> {
        if (identity.provinceId() == null) throw new BadRequestException("province scope missing");
        if (!identity.provinceId().equals(complaint.getIncidentProvinceId())) throw new ForbiddenException("out of scope");
      }
      case "DSD" -> {
        if (identity.districtId() == null) throw new BadRequestException("district scope missing");
        if (!identity.districtId().equals(complaint.getIncidentDistrictId())) throw new ForbiddenException("out of scope");
      }
      case "DS" -> {
        if (identity.dsDivisionId() == null) throw new BadRequestException("ds scope missing");
        if (!identity.dsDivisionId().equals(complaint.getIncidentDsDivisionId())) throw new ForbiddenException("out of scope");
      }
      case "GS" -> {
        if (identity.gnDivisionId() == null) throw new BadRequestException("gn scope missing");
        if (complaint.getIncidentGnDivisionId() == null || !identity.gnDivisionId().equals(complaint.getIncidentGnDivisionId())) {
